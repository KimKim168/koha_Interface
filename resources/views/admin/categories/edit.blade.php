...
@section('content')
    <div class="p-4">
        ...
        <form class="w-full" action="{{ url('admin/categories/' . $category->id) }}" method="POST">
            @method('PUT')
            @csrf
            <div class="grid md:grid-cols-2 md:gap-6">
                ...
                <div>
                    ...
                    <x-text-input id="name" class="block mt-1 w-full" type="text" name="name"
                        value="{{ $category->name }}" required autofocus placeholder="Name" />
                    <x-input-error :messages="$errors->get('name')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="name_kh" :value="__('messages.nameKh')" /><span class="text-red-500">*</span>
                    <x-text-input id="name_kh" class="block mt-1 w-full" type="text" name="name_kh"
                        value="{{ $category->name_kh }}" required autofocus placeholder="Name in Khmer" />
